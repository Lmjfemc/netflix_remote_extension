...
[Releases](https://github.com/Lmjfemc/netflix_remote_extension/releases)에서 최신 Windows x64 전체 ZIP을 내려받아 압축을 **모두** 풉니다. v0.3.0 파일명은 `netflix-lan-remote-v0.3.0-windows-amd64.zip`입니다.
...
v0.3.0의 페어링 유지·카탈로그 분리 변경은 **확장과 Go 보조 프로그램을 함께 업데이트**해야 합니다. 새 서버는 확장의 `hello` 메시지가 도착할 때까지 인증 요청에 503을 반환하고 임시 키를 발급하지 않습니다. 이전 확장만 남아 있으면 준비 상태가 완료되지 않으므로 확장 재로드도 확인하세요.
...
PC가 다른 Wi-Fi로 옮겨 가거나 IP가 바뀌면 서버가 3초 이내에 주소 목록을 다시 읽어 새 주소를 허용하고, 팝업과 `/pc` 페이지의 주소·QR도 갱신됩니다. 서버를 다시 켤 필요는 없지만 휴대폰은 새 주소로 다시 열어야 합니다.
...
**끄기**를 누르지 않았는데 연결이 끊기면(보조 프로그램 종료, 확장 서비스 워커 재시작 등) 확장이 1초부터 최대 30초 간격으로 늘려 가며 최대 10회 자동으로 다시 시작합니다. 팝업에 "서버 재연결 중"과 시도 횟수가 표시됩니다. 10회 모두 실패하면 자동 재연결을 멈추고 마지막 오류를 표시하므로 설치 상태를 확인한 뒤 **리모컨 켜기**를 다시 누르세요.

서버를 다시 켜도 **휴대폰 페어링은 유지됩니다.** 확장이 마지막 세션 키를 확장 전용 저장소에 보관했다가 보조 프로그램을 다시 시작할 때 넘겨주고, 휴대폰은 같은 키를 브라우저 저장소에 보관합니다. 페어링 코드는 재시작마다 새로 만들어지지만 이미 페어링된 휴대폰은 코드 없이 다시 연결됩니다. PC가 꺼져 있으면 휴대폰은 2초부터 최대 15초 간격으로 계속 다시 시도합니다.

새 코드를 발급하고 연결된 휴대폰을 끊으려면 팝업의 **연결 초기화**를 누르세요. 휴대폰 쪽에서는 **Unpair**가 저장된 키를 지웁니다. 한 번에 휴대폰 한 세션을 지원하며 새 연결이 기존 연결을 대체합니다.
...
- Netflix의 공식 리모컨 API가 아닌 내부 플레이어와 DOM을 사용하므로 Netflix 변경 시 수정이 필요할 수 있습니다. 재생 페이지에서 플레이어 API를 8초 이상 찾지 못하면 휴대폰 상태 줄에 **Netflix player API not found**가 표시됩니다. 이 메시지가 계속 보이면 Netflix 쪽 변경일 가능성이 높으므로 탭 새로고침 후에도 같다면 확장 업데이트가 필요합니다.
...
네트워크 소스 주소·Host·Origin을 검사하고, 페어링 시도 제한과 임의의 세션 토큰을 사용합니다. 세션 키는 PC의 확장 저장소와 휴대폰 브라우저 저장소에만 있으며 서버가 파일로 남기지 않고 `/setup` 응답에도 포함하지 않습니다. 페어링이 재시작 후에도 유지되므로 휴대폰을 잃어버렸거나 다른 사람이 페어링했다고 의심되면 팝업의 **연결 초기화**로 키를 바꾸세요. Netflix 자격증명을 수집하거나 별도 로그인하지 않습니다. 외부 연결 서버나 시청 기록 저장소도 없습니다.
